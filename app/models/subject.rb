...
class Subject < ActiveRecord::Base 
  ...
  has_many :timetable_entries,:foreign_key=>'subject_id', :dependent => :delete_all
  has_many :employees_subjects, :dependent => :delete_all
  has_many :employees ,:through => :employees_subjects, :dependent => :delete_all
  has_many :students_subjects, :dependent => :delete_all
  has_many :students, :through => :students_subjects, :dependent => :delete_all
  has_many :grouped_exam_reports, :dependent => :delete_all
  has_many :exams
...
#  validates_uniqueness_of :name, :case_sensitive => false, :scope=>[:batch_id,:is_deleted] ,:if=> 'is_deleted == false'
