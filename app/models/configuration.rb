...
class Configuration < ActiveRecord::Base
  ...
  STUDENT_ATTENDANCE_TYPE_OPTIONS = [["('daily_text')", "Daily"], ["('subject_wise_text')", "SubjectWise"]]

  NETWORK_STATES = [["('online')", 'Online'], ["('offline')", 'Offline']]
LOCALES = []
...
#def self.all
 #   Configuration.all # or replace with your own logic to fetch all configurations
  #end
  ...
  config = Configuration.find_by(config_key: key)
  if config.nil?
    ...
    default_country_value = find_by(config_key: 'DefaultCountry')&.config_value.to_i
